...
def detection_page():
    ...
    if st.button('Verify'):
        ...
        prediction_text = (
            'Alert: DENIAL OF SERVICE attack detected – immediate action required to mitigate impact.'
            if prediction == 0 else
            'Alert: Suspicious PROBING activity detected – potential reconnaissance in progress.'
            if prediction == 1 else
            'Alert: REMOTE-TO-LOCAL attack detected – unauthorized remote access attempt identified.'
            if prediction == 2 else
            'Alert: USER-TO-ROOT attack detected – suspicious privilege escalation detected.'
            if prediction == 3 else
            'You are SAFE! No anomalies or security threats identified.'
        )
